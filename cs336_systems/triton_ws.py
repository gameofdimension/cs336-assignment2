...
def weighted_sum_fwd(
    ...
    x_stride_row,
    x_stride_dim,  # Strides tell ushow to move one element in each axis of a tensoT
    ...
    # Each instance will compute the weighted sum of a tile of rows of w.
    # `tl.program_id` gives us a way to check which thread block we're running in
    row_tile_idx = tl.program_id(0)
    # Block pointers give us a way to select from an NDregion of memory
    # and move our selection around.
    # The block pointer must know:
    # - The pointer to the first element of the tensor
    # - The overall shape of the tensor to handle out-of-bounds access

    # - The strides of each dimension to use the memory layoutproperly
    # - The ND coordinates of the starting block, i.e., "offsets
    # - The block shape to use load/store at a time
    # - The order of the dimensions in memory from majorto minor
...
class WeightedSumFunc(torch.autograd.Function):
    ...
    def forward(ctx, x, weight):
        # Cache w and weight to be used in the backwaard pass, when we
...
def main():
    device = 'cuda'
    x = torch.randn(80, 140, device=device)
    w = torch.randn(140, device=device, requires_grad=True)

    f_weightedsum = WeightedSumFunc.apply

    y = f_weightedsum(x, w)
    gold = x @ w
    delta = (y - gold).abs().max().item()
    ...
    loss1 = y.sum()
    loss1.backward()
    grad_copy1 = w.grad.detach().clone()

    w.grad = None
    loss2 = gold.sum()
    loss2.backward()
    grad_copy2 = w.grad.detach().clone()

    grad_delta = (grad_copy1 - grad_copy2).abs().max().item()
