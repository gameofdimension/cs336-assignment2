...
def torch_fa_kernel(q, k, v, is_causal=False):
    ...
    for i in range(Tq):
        ...
        max_logit = torch.zeros(q_tile.shape[:-1], device=q_tile.device)
        torch.fill_(max_logit, float('-inf'))
        for j in range(Tk):
            k_tile = k[..., j * Bk:(j + 1) * Bk, :]
            v_tile = v[..., j * Bk:(j + 1) * Bk, :]
            s = einsum(q_tile, k_tile, '... q d, ... k d -> ... q k') / math.sqrt(d)
            max_s = torch.max(s, dim=-1).values
            new_max_logit = torch.max(max_logit, max_s)
            p = torch.exp(s - new_max_logit[..., None])
            adjustment = torch.exp(max_logit - new_max_logit)
        ...
        logsumexp[..., i * Bq:(i + 1) * Bq] = max_logit + torch.log(denominator)
